...
def parse_args(argv=None):
    ...
    parser.set_defaults(no_bar=False, display=False, resume=False, output_coco_json=False, output_web_json=False, shuffle=False,
                        ...
                        emulate_playback=False, greenscreen=False)
...
def evalvideo(net:Yolact, path, out_path:str=None):
    ...
    if is_webcam:
        ...
        ffmpeg_cmd =[FFMPEG_BIN, '-i', 'srt://192.168.1.106:1935?streamid=output/live/atl', '-r', '24', '-pix_fmt', 'bgr24', '-vcodec', 'rawvideo', '-an', '-sn', '-f', 'image2pipe', '-']
        pipe = sp.Popen(ffmpeg_cmd, stdout = sp.PIPE, bufsize=10**8)
        raw_image = pipe.stdout.read(IMG_W*IMG_H*3)
        image =  np.frombuffer(raw_image, dtype='uint8')
        vid = image.reshape((IMG_H,IMG_W,3))
    ...
    if is_webcam:
        ...
        if "rtmp" in path or "srt" in path:
            num_frames = float('inf')
        else:
